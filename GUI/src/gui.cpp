...
static std::string GetSystemFontFile(const std::string& faceName) {
	...
	HRESULT ret = GetWindowsDirectory(winDir, MAX_PATH);
...
void GUI::Init(const wchar_t* windowName, int w, int h) {
	...
	mainFontCfg.OversampleH = 3.f;
	mainFontCfg.OversampleV = 3.f;
	defFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(arial.c_str(), 16.0f, &mainFontCfg, ImGui::GetIO().Fonts->GetGlyphRangesCyrillic());
...
void GUI::endFrame() {
	//Watermark
	ImGui::GetForegroundDrawList()->AddText(ImVec2(15,15), IM_COL32(255, 255, 255, 100), "Made by Filip Nejedly at UTB 2022", NULL);
